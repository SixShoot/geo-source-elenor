...
extends AMovementMotor {
    ...
    public boolean update() {
        ...
        if (target == null || this._task != null && this._task.isCancelled() || this._owner.getLifeStats().isAlreadyDead() || this._owner.getAi2().getState() == AIState.DIED) {
            return false;
        }
        ...
        if (this.canMove() && !canPass && pathfindRevalidationTime < System.currentTimeMillis()) {
            ...
            float newZ = GeoService.getInstance().getZ(this._owner.getWorldId(), target.getX(), target.getY(), target.getZ(), 100.0f, this._owner.getInstanceId());
            ...
            double distance = GeomUtil.getDistance3D(this._lastMovePoint, getTargetPos.x, getTargetPos.y, getTargetPos.z) - Math.max(range, this._owner.getCollision());
        ...
        if (this._targetPosition != null) {
            directionChanged = this._targetPosition.x != this.targetDestX || this._targetPosition.y != this.targetDestY || this._targetPosition.z != this.targetDestZ;
            this.targetDestX = this._targetPosition.x;
            this.targetDestY = this._targetPosition.y;
            this.targetDestZ = this._targetPosition.z;
            double distance = GeomUtil.getDistance3D(this._owner.getX(), this._owner.getY(), this._owner.getZ(), this._targetPosition.x, this._targetPosition.y, this._targetPosition.z);
            ...
            this.new_targetHeading = (byte)(Math.toDegrees(Math.atan2(this._targetPosition.y - this._owner.getY(), this._targetPosition.x - this._owner.getX())) / 3.0);
            if (directionChanged) {
                PacketSendUtility.broadcastPacket(this._owner, new SM_MOVE(this._owner.getObjectId(), this._owner.getX(), this._owner.getY(), this._owner.getZ(), this._targetPosition.x, this._targetPosition.y, this._targetPosition.z, this.new_targetHeading, this._targetMask));
            }
        ...
        this._task = this._processor.schedule(new Runnable(){
            ...
            public void run() {
                if (FollowMotor.this._targetPosition != null) {
                    ...
                    Vector3f targetMove = new Vector3f(FollowMotor.this._targetPosition.x, FollowMotor.this._targetPosition.y, FollowMotor.this._targetPosition.z);
                    ...
                    World.getInstance().updatePosition(FollowMotor.this._owner, position.x, position.y, position.z, FollowMotor.this.new_targetHeading, false);
                } else {
    ...
    private boolean canMove() {
        return !this._owner.getEffectController().isUnderFear() && this._owner.canPerformMove() && this._owner.getAi2().getSubState() != AISubState.CAST;
    }
}