...
public class AGameProcessor {
    ...
    public ScheduledFuture<?> schedule(Runnable r, long delay) {
        ...
        if (validated < delay) {
            Log.warn("Determine attempt to post scheduled task with delay {}, but maximal is {}. Delay will be trimmed to maximal", (Object)delay, (Object)validated);
        }
        delay = validated;
    ...
    public boolean schedule(Runnable r, long delay, Task out) {
        ...
        long validated = Math.max(0L, Math.min(Integer.MAX_VALUE, delay));
        if (validated < delay) {
            Log.warn("Determine attempt to post scheduled task with delay {}, but maximal is {}. Action will not be triggered", (Object)delay, (Object)validated);
            return false;
        }
        delay = validated;
    ...
    private static final class RunnableTaskWrapper
