...
extends ActionObserver {
    ...
    private void shedulesEvent() {
        int delay = this.template != null && this.template.getDuration() >= 1000 ? this.template.getDuration() : (this.mathObject.getDuration() >= 1000 ? this.mathObject.getDuration() : 1000);
    ...
    private void onActionEvent() {
        switch (this.type) {
            case SKILL_USE: {
                if (this.creature.getEffectController().hasAbnormalEffect(this.mathObject.getSkillId())) break;
                if (this.template == null) {
                    return;
                }
                SkillEngine.getInstance().applyEffectDirectly(this.mathObject.getSkillId(), this.mathObject.getMaster(), this.creature, this.template.getDuration());
                break;
            }
        }
    }
