...
extends VisibleObjectController<MathObject> {
    FastMap<Creature, MathObjectObserver> observers = new FastMap().shared();

    @Override
    public void see(VisibleObject object) {
        super.see(object);
        if (((MathObject)this.getOwner()).getReaction() == MathObjectReaction.PC ? !(object instanceof Player) : (((MathObject)this.getOwner()).getReaction() == MathObjectReaction.NPC ? !(object instanceof Npc) : ((MathObject)this.getOwner()).getReaction() == MathObjectReaction.ALL && !(object instanceof Creature))) {
            return;
        }
        Creature creature = (Creature)object;
        MathObjectObserver observer = new MathObjectObserver((MathObject)this.getOwner(), creature, ((MathObject)this.getOwner()).getType());
...
/*
    public Npc spawn(int worldId, int npcId, float x, float y, float z, byte heading, int staticId, int instanceId, int randomWalk, String ai) {
        SpawnTemplate template = SpawnEngine.addNewSingleTimeSpawn((int)worldId, (int)npcId, (float)x, (float)y, (float)z, (byte)heading, (int)randomWalk);
        template.setStaticId(staticId);
        Npc master = (Npc)SpawnEngine.spawnObject(template, instanceId);
        if (randomWalk > 0) {
            if (master.getObjectTemplate().getStatsTemplate().getWalkSpeed() == 0.0f) {
                // empty if block
            }
            WalkManager.startWalking((NpcAI2)master.getAi2());
        }
        if (ai != null) {
            AI2Engine.getInstance().setupAI(ai, (Creature)master);
            ((NpcAI2)master.getAi2()).setStateIfNot(AIState.IDLE);
        }
        ((MathObject)this.getOwner()).setMaster(master);
        return ((MathObject)this.getOwner()).getMaster();
    }
*/
    ...
    public void delete() {
        ...
        ((MathObject)this.getOwner()).getKnownList().doOnAllObjects(new Visitor<VisibleObject>(){
            ...
            public void visit(VisibleObject object) {
                ...
                Creature creature = (Creature)object;
                MathObjectObserver observer = (MathObjectObserver)MathController.this.observers.remove((Object)creature);
                if (observer == null) {
                    return;
                }
                observer.clearShedules();
